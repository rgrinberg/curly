...
let callback req body = function
  | [ "simple_get" ] -> Server.respond_string ~status:`OK ~body:simple_get_body ()
  | [ "read_header" ] ->
    Server.respond_string
      ...
      ~body:
        (match Header.get (Request.headers req) "x-curly" with
         ...
         | Some v -> v)
      ()
  | [ "write_body" ] -> Server.respond_string ~status:`OK ~body ()
  | _ -> failwith (sprintf "Not found: %s" (Request.resource req))
;;
  ...
  Server.make
    ~callback:(fun _ req body ->
      let uri_parts =
        ...
        |> List.tl
      in
      Cohttp_lwt.Body.to_string body >>= fun body -> callback req body uri_parts)
    ()
...
let run_simple_get _ =
  Alcotest.check
    t_result
    "simple_get"
    (Ok { Response.code = 200; body = "curly"; headers = body_header simple_get_body })
...
let read_header _ =
  ...
  Alcotest.check
    t_result
    "read_header"
    (Ok { Response.code = 200; body = v; headers = body_header v })
...
|} in
  Alcotest.check
    t_result
    "write_body"
    (Ok { Response.code = 200; body; headers = body_header body })
    ...
    Lwt_preemptive.detach
      (fun () ->
        Alcotest.run
          "curly"
          [ ( "curly"
            , [ "simple_get", `Quick, run_simple_get
      ...
      ()
  in
  tests_done >|= (fun () -> Lwt.wakeup stopper ()) |> Lwt_main.run
;;
